...
class EventAnalyzer(Analyzer):
    ...
    def _continuous_messages(self) -> list[tuple[str, list[float | int]]]:
        freq = self._freq_from_com()
        amp = min(max(self.pose_speed * 0.85, 0.0), 0.85)
        cutoff = round(280.0 + self.flow_energy * 4200.0, 4)
        pan = round((self.com[0] - 0.5) * 2.0, 4)
        reverb_mix = round(0.05 + self.flow_energy * 0.75, 4)
        room = round(SC_REVERB_ROOM_MIN + self.depth_com * (SC_REVERB_ROOM_MAX - SC_REVERB_ROOM_MIN), 4)
        delay_time = round(
            SC_DELAY_TIME_MIN
            + ((self.flow_direction + math.pi) / (2.0 * math.pi)) * (SC_DELAY_TIME_MAX - SC_DELAY_TIME_MIN),
            4,
        )
        delay_feedback = round(min(0.15 + self.flow_energy * 0.55, 0.9), 4)
        return [
